...
@celery_app.task(name="tasks.periodic_tasks.discover_jobs")
def discover_jobs():
    """
    Periodic task to discover and queue eligible jobs.
    This runs every hour via Celery Beat.
    """
    logger.info("Running job discovery task")
    ...
    with SessionLocal() as session:
        ...
        try:
            ...
            logger.info(f"Found {len(eligible_jobs)} eligible jobs")
            
            for job in eligible_jobs:
                logger.info(f"Queueing job: {job.job_name} (ID: {job.job_id})")
                ...
                job_run = repo_factory.job_run.create(job_run_data)
...
@celery_app.task(name="tasks.simple_task.health_check")
def health_check():
